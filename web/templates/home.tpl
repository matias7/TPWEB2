      ...
      <div class="container">
        ...
        <ul class="list-group">
            {foreach from=$Productos item=producto}
              <li class="list-group-item">Producto: {$producto['Nombre']} - Valor: $ {$producto['Precio']} - Cantidad: {$producto['Unidades']} - Tipo: {$producto['nombre_categoria']}<a href="editarProducto/{$producto['nombre_categoria']}"> <button  type="button"  class="btn btn-warning">Editar</button></a> <a href="borrarProducto/{$producto['nombre_categoria']}"><button type="button" class="btn btn-danger">Borrar</button></a></li>
            {/foreach}
        </ul>
      </div>
